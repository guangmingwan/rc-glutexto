		...
		case SDL_JOYBUTTONDOWN:
											  ...
											  (event.jbutton.button >= SP_AXIS_UP && event.jbutton.button <= SP_AXIS_RIGHTUP))))
			{
				...
				if (sp_touchscreen_emulation)
				{
					...
					{
					case SP_AXIS_RIGHT:
					case SP_AXIS_RIGHTDOWN:
					case SP_AXIS_RIGHTUP:
						sp_touchscreen_dx = +1;
						break;
					case SP_AXIS_LEFT:
					case SP_AXIS_LEFTDOWN:
					case SP_AXIS_LEFTUP:
						sp_touchscreen_dx = -1;
						break;
					default:
...
//GCW and DINGUX use the "keyboard" for buttons
		...
		case SDL_JOYAXISMOTION:
			...
			else if (event.jaxis.value > SP_JOYSTICK_MAX_TRIGGER_ON && sp_axis_was_used[event.jaxis.axis & 1] != 1)
			{
				spGenericInput.axis[event.jaxis.axis & 1] = 1;
				sp_axis_was_used[event.jaxis.axis & 1] = 1;
				spLastAxisType = 1;
			}
			else if (event.jaxis.value > SP_JOYSTICK_MIN_TRIGGER_OFF && event.jaxis.value < SP_JOYSTICK_MAX_TRIGGER_OFF && sp_axis_was_used[event.jaxis.axis & 1] != 0)
			{
...
inline void spUpdateAxis(int axis)
{
	...
	else if (spGenericInput.axis[axis] == 1)
...
PREFIX void spScale2XSmooth(SDL_Surface *source, SDL_Surface *destination)
{
	...
	A = P;
	B = P;
	C = spScaleC((src_w - 1), 0);
	D = spScaleD((src_w - 1), 0);
	...
	for (y = 1; y < source->h - 1; y++)
	{
		...
		A = spScaleA((src_w - 1), y);
		B = P;
		C = spScaleC((src_w - 1), y);
		D = spScaleD((src_w - 1), y);
		spScalePixel((src_w - 1) * 2, Y);
	}
	//Y = n
	P = spScaleP(0, (source->h - 1));
	A = spScaleA(0, (source->h - 1));
	B = spScaleB(0, (source->h - 1));
	C = P;
	D = P;
	...
	for (x = 1; x < src_w - 1; x++)
	{
		...
		A = spScaleA(x, (source->h - 1));
		B = spScaleB(x, (source->h - 1));
		C = spScaleC(x, (source->h - 1));
		D = P;
		spScalePixel(X, (source->h - 1) * 2);
	}
	P = spScaleP((src_w - 1), (source->h - 1));
	A = spScaleA((src_w - 1), (source->h - 1));
	B = P;
	C = spScaleC((src_w - 1), (source->h - 1));
	D = P;
...
PREFIX void spSetVirtualKeyboard(int state, int x, int y, int width, int height, SDL_Surface *design, SDL_Surface *shiftDesign)
{
	...
	switch (state)
	{
	case SP_VIRTUAL_KEYBOARD_NEVER:
		spVirtualKeyboardState = SP_VIRTUAL_KEYBOARD_NEVER;
		break;
#if defined PANDORA || defined DESKTOP
	case SP_VIRTUAL_KEYBOARD_IF_NEEDED:
		spVirtualKeyboardState = SP_VIRTUAL_KEYBOARD_NEVER;
		break;
#else
	case SP_VIRTUAL_KEYBOARD_IF_NEEDED:
		...
		break;
#endif
	case SP_VIRTUAL_KEYBOARD_ALWAYS:
		spVirtualKeyboardState = SP_VIRTUAL_KEYBOARD_ALWAYS;
		break;
	}
